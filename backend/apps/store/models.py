...
class SiteSettings(models.Model):
    ...
    # ---- برند سایت (نشست ۴۹) ----
    # همان یک تصویر، هم به‌جای باکسِ ثابتِ «BM» در هدر/فوتر گذاشته می‌شود
    # هم به‌عنوان فاویکونِ سایت — طبقِ خواسته‌ی شما، عمداً یک فیلد است نه
    # دو تا؛ برای فاویکون معمولاً یک تصویرِ مربعیِ ساده (بدونِ متنِ ریز)
    # بهتر جواب می‌دهد، ولی این‌جا محدودیتی رویش اعمال نشده — هر تصویری
    # که فروشنده وارد کند، هم در هدر (با اندازه‌ی بزرگ‌تر) هم در تبِ
    # مرورگر (با اندازه‌ی خیلی کوچک) استفاده می‌شود.
    brand_image = models.ImageField(
        "لوگو / برند سایت",
        upload_to="branding/",
        blank=True,
        null=True,
        help_text="همین یک تصویر هم به‌جای لوگوی هدر/فوتر سایت و هم به‌عنوان "
        "فاویکونِ تبِ مرورگر استفاده می‌شود. ترجیحاً یک تصویرِ مربعیِ ساده "
        "(بدونِ متنِ ریز که در اندازه‌ی کوچکِ فاویکون ناخوانا می‌شود).",
    )

    # ---- شرایط و قوانین (نشست ۵۰) ----
    # متنِ مودالِ «شرایط و قوانین» در صفحه‌ی ورود/ثبت‌نام. عمداً یک
    # فیلدِ متنی روی همین singleton (نه یک صفحه‌ی جدا یا یک فایلِ ثابت
    # در تمپلیت) — چون متن کوتاه است و صفحه‌ی جداگانه لازم نبود، ولی
    # چون این پروژه از قبل هر متنِ قابل‌ویرایشِ دیگری (هیروها، معرفیِ
    # فروشگاه، درباره‌ی ما، ...) را همین‌جا نگه می‌دارد، اگر بعداً لازم
    # شد متنِ حقوقی عوض شود، فروشنده از پنلِ ادمین این کار را می‌کند، نه
    # با ویرایشِ کد. مقدارِ پیش‌فرض همان متنی است که شما دادید — یعنی
    # نیازی نیست برای اولین‌بار هم چیزی در ادمین وارد کنید.
    terms_text = models.TextField(
        "متن شرایط و قوانین",
        blank=True,
        default=(
            "با ثبت سفارش در بانک مکمل، تأیید می‌نمایم که مشخصات و اطلاعات مربوط به "
            "محصول، از جمله ترکیبات، نحوه مصرف، شرایط نگهداری، هشدارها و محدودیت‌های "
            "مربوط به آن را مطالعه کرده و با آگاهی از اطلاعات ارائه‌شده، اقدام به خرید "
            "می‌نمایم.\n\n"
            "همچنین متعهد می‌شوم محصولات خریداری‌شده را مطابق دستورالعمل و اطلاعات "
            "درج‌شده بر روی محصول و توضیحات ارائه‌شده در بانک مکمل مورد استفاده قرار دهم.\n\n"
            "ثبت سفارش در بانک مکمل به منزله مطالعه و پذیرش شرایط و قوانین استفاده از "
            "سایت، شرایط خرید و پرداخت، نحوه ارسال، شرایط مرجوعی و خدمات پس از فروش "
            "می‌باشد."
        ),
        help_text="در مودالِ «شرایط و قوانین» صفحه‌ی ورود/ثبت‌نام نمایش داده می‌شود. "
        "هر پاراگراف را با یک خط خالی از پاراگرافِ بعدی جدا کنید.",
    )
